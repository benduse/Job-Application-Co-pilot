    ...
    const handleFile = useCallback(async (file: File) => {
        ...
        if (extension === 'txt') {
            ...
            try {
                ...
                alert(err.message || 'Failed to extract text from the document. Please try a different file or paste the text manually.');
            } finally {
    ...
    return (
        <div className="bg-slate-800/50 p-6 rounded-lg border border-slate-700 h-full flex flex-col">
            ...
            <div
                ...
                onDrop={handleDrop}
                className={`relative border-2 border-dashed rounded-lg p-8 text-center transition-colors duration-200 ${isDragging || isExtractingText ? 'border-cyan-500 bg-slate-800' : 'border-slate-600 hover:border-cyan-600'}`}
            >
                {isExtractingText ? (
                    <div className="flex flex-col items-center justify-center text-slate-400 pointer-events-none">
                        <Loader />
                        <p className="mt-2 font-semibold">Extracting text from document...</p>
                    </div>
                ) : (
                    <>
                        <input
