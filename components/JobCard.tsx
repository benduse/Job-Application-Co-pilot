...
interface JobCardProps {
    job: Job;
    resume: string;
    onJobChange: (id: string, updates: Partial<Pick<Job, 'url' | 'description'>>) => void;
...
const JobCard: React.FC<JobCardProps> = ({ job, resume, onJobChange, onFetch, onAnalyze, onRemove, isAnalyzeDisabled, onSelectSuggestion }) => {
    ...
    return (
        <div className="bg-slate-800/50 border border-slate-700 rounded-lg shadow-lg overflow-hidden transition-all duration-300">
            <div className="p-4 space-y-4">
                 <div className="flex justify-between items-start">
                    <div className="flex-1 pr-4">
                        <h3 className="text-xl font-bold text-white">{job.title || 'New Job Listing'}</h3>
                        <p className="text-sm text-slate-400">{job.company || 'Enter company name and job URL'}</p>
                    </div>
                    <button
                 ...
                 <div>
                    <label htmlFor={`job-url-${job.id}`} className="block text-sm font-medium text-slate-300 mb-2">
                        Job Posting URL
                    </label>
                    <div className="flex gap-2">
                        <input
                            ...
                            className={`flex-grow bg-slate-800 border rounded-md shadow-sm p-3 text-slate-200 placeholder-slate-500 focus:ring-2 transition duration-150 ease-in-out ${showUrlError ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-slate-600 focus:ring-cyan-500 focus:border-cyan-500'}`}
                            aria-invalid={showUrlError}
                            aria-describedby={showUrlError ? `job-url-error-${job.id}` : undefined}
                        />
                        <button
                    ...
                    {showUrlError && (
                        <p id={`job-url-error-${job.id}`} className="mt-2 text-sm text-red-400">
            ...
            {job.analysis && <div className="p-4 border-t border-slate-700"><MatchResult analysis={job.analysis} resume={resume} onSelectSuggestion={onSelectSuggestion} /></div>}
        </div>
    );
};
