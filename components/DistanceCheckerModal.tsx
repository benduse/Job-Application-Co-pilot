...
const DistanceCheckerModal: React.FC<DistanceCheckerModalProps> = ({
	onClose,
}) => {
		...
		>
			...
			>
					...
					<div>
						...
						<div className='flex gap-2'>
							...
							<button
								...
								className='flex-shrink-0 flex items-center justify-center px-3 py-2 text-xs font-bold text-white bg-slate-600 rounded-md hover:bg-slate-500 disabled:bg-slate-700 disabled:cursor-not-allowed transition-colors'
							>
					...
					{result && (
						<div className='bg-slate-900/50 text-green-300 p-4 rounded-md text-center border border-slate-700'>
							<p className='text-sm text-slate-400'>
								The driving distance between <br />
							...
							<p className='text-3xl font-bold text-green-400 mt-2'>
								{result.distance.toFixed(1)} {result.unit}
							</p>
						</div>
					)}
				</div>
				<div className='px-6 py-4 bg-slate-900/50 rounded-b-lg flex justify-end items-center gap-4'>
					<button
						onClick={onClose}
						className='px-5 py-2 text-sm font-bold text-slate-300 bg-slate-600 rounded-md hover:bg-slate-500 transition-colors'
					>
						Close
					</button>
					<button
						...
						className='flex items-center justify-center w-40 px-5 py-2 text-sm font-bold text-white bg-cyan-600 rounded-md hover:bg-cyan-500 transition-colors disabled:bg-slate-600 disabled:cursor-not-allowed'
					>
