...
const AnalysisSection: React.FC<{ title: string; children: React.ReactNode; icon: React.ReactNode; defaultOpen?: boolean }> = ({ title, children, icon, defaultOpen = false }) => {
    ...
    return (
        <div className="bg-slate-800/50 rounded-lg border border-slate-700">
             <button onClick={() => setIsOpen(!isOpen)} className="w-full p-4 flex justify-between items-center text-left">
                ...
                <span className={`transform transition-transform duration-200 ${isOpen ? 'rotate-180' : 'rotate-0'}`}><ChevronDownIcon /></span>
             </button>
...
const KeywordCloud: React.FC<{ keywords: Keyword[], color: 'green' | 'red' }> = ({ keywords, color }) => {
    const colorClasses = {
...
const HighlightedResume: React.FC<{ resume: string; keywords: string[] }> = ({ resume, keywords }) => {
    const highlightedText = useMemo(() => {
        ...
        return resume.split(regex).map((part, index) => 
            regex.test(part) ? <mark key={index} className="bg-green-500/30 text-green-200 px-1 rounded">{part}</mark> : part
        );
    }, [resume, keywords]);

    return <pre className="whitespace-pre-wrap font-sans text-sm bg-slate-900 p-4 rounded-md border border-slate-700">{highlightedText}</pre>
};


const MatchResult: React.FC<MatchResultProps> = ({ analysis, resume, onSelectSuggestion }) => {
  ...
  const scoreColor = analysis.matchScore >= 75 ? 'text-green-400' : analysis.matchScore >= 50 ? 'text-yellow-400' : 'text-red-400';
    ...
    return (
        <div className="relative flex items-center justify-center w-32 h-32">
          <svg className="transform -rotate-90" width="120" height="120" viewBox="0 0 120 120">
            <circle cx="60" cy="60" r={radius} strokeWidth="10" className="text-slate-700" fill="transparent" stroke="currentColor" />
            <circle
              cx="60"
              cy="60"
              r={radius}
  ...
  return (
    <div className="space-y-6">
      ...
      <AnalysisSection title="Keyword Analysis" icon={<KeyIcon />} defaultOpen={true}>
        <div className="space-y-4">
            <div>
             ...
             <div>
                <h4 className="font-semibold text-red-400 mb-2">Missing Keywords (Gaps)</h4>
                <KeywordCloud keywords={analysis.missingKeywords} color="red" />
            </div>
        </div>
      </AnalysisSection>

      <AnalysisSection title="Actionable Suggestions" icon={<MagicWandIcon />} defaultOpen={true}>
        <div className="space-y-3">
            {analysis.improvementSuggestions.map((suggestion, index) => (
                <div key={index} className="bg-slate-900/70 p-3 rounded-md border border-slate-700 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                    <div className="flex-1">
                        <span className="text-xs font-semibold uppercase text-indigo-400">{suggestion.suggestionType}</span>
                        <p className="text-sm text-slate-400 italic mt-1">"{suggestion.originalText}"</p>
                    </div>
                    <button onClick={() => onSelectSuggestion(suggestion)} className="px-4 py-1.5 text-sm font-bold text-white bg-indigo-600 rounded-md hover:bg-indigo-500 transition-colors flex-shrink-0">Review & Apply</button>
                </div>
            ))}
        </div>
      </AnalysisSection>
       ...
       <AnalysisSection title="Highlighted Resume" icon={<DocumentTextIcon />}>
            <HighlightedResume resume={resume} keywords={analysis.matchedKeywords.map(k => k.keyword)} />
       </AnalysisSection>
...
const CheckIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
const AlertIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-yellow-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>;
const KeyIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H5v-2H3v-2H1v-4a6 6 0 016-6h7z" /></svg>;
...
const ChevronDownIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>;
